...
if(isset($_GET['fecha'])){
    ...
    $ejecutar_inv_contar = mysqli_query($con, $consulta_inv_contar); 
    $row = mysqli_fetch_array($ejecutar_inv_contar);
    if($row['numero'] == 0){//en caso de que no llegue ningun dato manda este array para que con ajax mande un mensaje diciendo que no hay registros con esa fecha
        ...
        $ejecutar_inversion = mysqli_query($con, $consulta_inversion);
    
        while($row = mysqli_fetch_array($ejecutar_inversion)) {        
            $inversion[] = array(
                'id_inv' => $row['id_inv'],
    ...
    $ejecutar_gastos_contar = mysqli_query($con, $consulta_gastos_contar); 
    $row = mysqli_fetch_array($ejecutar_gastos_contar);
    if($row['numero'] == 0){
        $gastos[] = array(
            'id_gastos' => 0,
        );
        ...
        $ejecutar_gastos = mysqli_query($con, $consulta_gastos);   
        while($row = mysqli_fetch_array($ejecutar_gastos)) {   
            $gastos[] = array(
    ...
    $ejecutar_venta = mysqli_query($con, $consulta_venta);
    if($ejecutar_venta){
        while($row = mysqli_fetch_array($ejecutar_venta)) {
