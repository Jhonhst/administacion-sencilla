...
if(isset($_POST['tiempo'])){
    ...
    $consulta = (//trae los datos dependiendo el tiempo que se le ponga
        "SELECT DATE_FORMAT(I.FECHA, '%d-%m-%Y') AS LAFECHA, INVERSION_VALOR, GASTO_VALOR, TOTAL_VENTA 
        FROM (SELECT FECHA, SUM(VALOR) AS INVERSION_VALOR FROM INVERSION GROUP BY (FECHA) DESC)AS I
        JOIN 
        (SELECT FECHA, SUM(VALOR) AS GASTO_VALOR FROM GASTOS GROUP BY(FECHA) DESC)AS G
        ON G.FECHA = I.FECHA 
        JOIN 
        (SELECT FECHA, TOTALVENTA AS TOTAL_VENTA FROM VENTA GROUP BY(FECHA) DESC)AS V 
    ...
    $consulta = (
        "SELECT DATE_FORMAT(I.FECHA, '%d-%m-%Y') AS LAFECHA, INVERSION_VALOR, GASTO_VALOR, TOTAL_VENTA 
        FROM (SELECT FECHA, SUM(VALOR) AS INVERSION_VALOR FROM INVERSION  WHERE fecha >= '$fecha_inicio' AND fecha <= '$fecha_fin' GROUP BY (FECHA) DESC)AS I
        JOIN 
        (SELECT FECHA, SUM(VALOR) AS GASTO_VALOR FROM GASTOS WHERE fecha >= '$fecha_inicio' AND fecha <= '$fecha_fin' GROUP BY(FECHA) DESC)AS G
        ON G.FECHA = I.FECHA 
        JOIN 
        (SELECT FECHA, TOTALVENTA AS TOTAL_VENTA FROM VENTA WHERE fecha >= '$fecha_inicio' AND fecha <= '$fecha_fin' GROUP BY(FECHA) DESC)AS V 
        ON V.FECHA = I.FECHA"
    );
    $ejecutar = mysqli_query($con,$consulta) or die(mysqli_error($con));
    ...
    $consulta = (//debo hacer una cosulta general pero para la triple selección se deben hacer individualmente, debido a que hay tablas que tienen mas datos que otras, eso hace que algunos datos se sumen de las tablas que tienen menos datos.
        "SELECT DATE_FORMAT(I.FECHA, '%d-%m-%Y') AS LAFECHA, INVERSION_VALOR, GASTO_VALOR, TOTAL_VENTA
        FROM (SELECT FECHA, SUM(VALOR) AS INVERSION_VALOR FROM INVERSION  WHERE fecha = '$fecha_unica' GROUP BY (FECHA) DESC)AS I
        JOIN 
        (SELECT FECHA, SUM(VALOR) AS GASTO_VALOR FROM GASTOS WHERE fecha = '$fecha_unica' GROUP BY(FECHA) DESC)AS G
        ON G.FECHA = I.FECHA 
        JOIN 
        (SELECT FECHA, TOTALVENTA AS TOTAL_VENTA FROM VENTA WHERE fecha = '$fecha_unica' GROUP BY(FECHA) DESC)AS V 
        ON V.FECHA = I.FECHA"
    );
